...
const useExports = () => {
    ...
    const exportToExcel = async (data: any[], columnFields: { field: string, label?: string, title?: string }[]) => {
        ...
        const startExport = new Promise((resolve, reject) => {
            saveAs(new Blob([s2ab(output)], { type: "application/octet-stream" }), `${date.getUTCMonth()}${date.getUTCDay()}${date.getUTCFullYear()}_${date.getUTCMinutes()}_export.xlsx`)
            resolve('done');
            reject('error');
        });
    ...
    const exportToPDF = async (data: any[], columnFields: { field: string, label?: string, title?: string }[]) => {
        ...
        (doc as any).autoTable({
            ...
            headStyles: { fontSize: 7, wordBreak: "break-word", fillColor: '#222E3E' },
